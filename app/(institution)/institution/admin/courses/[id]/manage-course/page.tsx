...
//   // ✅ FIX: Pass 'document' instead of 'file'
//   const resource_type = isFile ? 'document' : 'url';
//   const resource_Url = isFile ? '' : resourceUrlOrFile;

//   setIsSavingLecture(true);

//   try {
//     const success = await addReources(
//       selectedSectionForLecture,
//       resource_type,
//       resource_Url,
//       resourceTitle,
...
const handleResourceSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
  ...
  try {
    ...
    if (isEditing) {
      // EDIT mode - call editResources
      success = await editReources(
     resourceId,
        selectedSectionForLecture,
      ...
      // NEW resource - call addResources
      success = await addReources(
        selectedSectionForLecture,
        resource_type,
        resource_Url,
        resourceTitle,
...
const handleQuizANS = async (
  ...
  try {
    ...
    if (isEditing && selectedQuiz) {
      ...
      await updateQAns(
        selectedQuiz.question_id, // Pass the question_id for updating specific question
        selectedQuiz.quiz_id,     // Pass the quiz_id the question belongs to
        data.question_text,
        data.answers
      );
      ...
      // Handle create
      await addQAns(sectionId, data.question_text, data.answers);
...
/>
  ...
  //       prev
  //         ? { ...prev, title: value }
  //         : { title: value, description: '', class_date: '', class_time: '' }
  //     )
  //   }
  ...
  //     setSelectedLecture(prev => (prev ? { ...prev, duration } : null))
  //   }
  //   isSaving={isSavingLecture}
  //   videoFile={videoFile}
