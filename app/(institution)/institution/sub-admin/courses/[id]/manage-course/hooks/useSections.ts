...
const addReources = async (
  ...
  try {
    ...
    // ✅ Always append 'resource_url'
  if (resource_type === 'document' && resource_file) {
  ...
  formData.append('resource_url', resource_Url);
    ...
    throw new Error(error?.response?.data?.message || 'Error adding lecture resource');
  }
};
const editReources = async (
  rsource_id:String,
  lecture_id: string,
  resource_type: string,
  resource_Url: string,
  ...
  formData.append('resource_url', resource_Url);
    ...
    const response = await axios.post(
      `${baseUrl}/recorded-lecture-resources/update/${rsource_id}`,
      formData,
      {
        ...authorizationObj,
        headers: {
