...
interface Notification {
  ...
  sent_to: string | null;
  created_at: string;
  sent_at?: string; // Add sent_at as optional for compatibility
}
  ...
  return (
    <div className="px-0">
      {notifications.length === 0 ? (
        <div className="text-start text-muted py-4">No Received notifications.</div>
      ) : (
              ...
              <div key={n.id || idx}>
                <div className="notification-row align-items-center mb-3">
                  ...
                  <div className="notification-date">
                    {format(new Date(n.sent_at || n.created_at), "MMM dd, yyyy hh:mm a")}
                  </div>
                </div>
              </div>
            );
