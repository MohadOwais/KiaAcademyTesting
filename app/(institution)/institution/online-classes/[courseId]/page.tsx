...
const CourseDetails = () => {
  ...
  const [price, setPrice] = useState<PriceMatrix[]>([]);
  ...
  const handleDelete = async () => {
    try {
      ...
      if (response.data.status === 200) {
        ...
        router.push('/tutor/online-classes');
      } else {
    ...
    <>
      <Container fluid className="mt-4 px-4">
        ...
        <div className="d-flex justify-content-between align-items-center mb-4">
          <Link href="/tutor/online-classes" className="text-decoration-none">
          ...
          <div className="d-flex gap-2">
            <Link href={`/tutor/online-classes/${courseId}/manage-course`} className='text-decoration-none'>
              <Button className="d-flex align-items-center btn-view rounded-pill ">
                <i className="bi bi-list me-2"></i>
                Manage Course
              </Button>
            </Link>
            <Link href={`/tutor/online-classes/${courseId}/edit`} className='text-decoration-none'>
              <Button className="d-flex align-items-center btn-view rounded-pill">
                          ...
                          <ul className="list-unstyled">
                            ...
                            <li className="mb-3 d-flex align-items-center">
                              ...
                              <span className="badge bg-primary">
                                {(() => {
                                  const selectedPrice = price.find((p: PriceMatrix) => p.ID === String(course.course_price));

                                  if (!selectedPrice) return 'Price Not Found';

                                  const priceValue = selectedPrice.tier_price.trim().split(' ')[0];

                                  return priceValue === '0' || priceValue === '0.00'
                                    ? 'Free'
                                    : `${course.display_currency} ${selectedPrice.tier_price}`;
                                })()}
                              </span>
                            </li>
                            <li className="mb-3 d-flex align-items-center">
                              ...
                              <span
                                className={`badge ${course.course_status === '1' ? 'bg-success' : 'bg-danger'
                                  }`}
                              >
                      ...
                      <Card className="h-100 border-0 bg-light">
                        <Card.Body className="p-4">
                          ...
                          <ul className="list-unstyled">
                            ...
                            <li className="mb-3 d-flex align-items-center">
                              <i className="bi bi-clock me-2 text-primary"></i>
                              <strong className="me-2">Timezone:</strong>
                              <span>{course.time_zone}</span>
                            </li>
                          </ul>
                        </Card.Body>
                      </Card>
