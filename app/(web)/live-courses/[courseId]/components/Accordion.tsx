  ...
  return (
    <div className="w-100 d-flex align-items-start gap-2 px-4">
      ...
      <p className="w-100 text-start m-0">
        {medium} are locked, You have to enroll first
      </p>
    </div>
  );
};
...
export const Level3 = ({
  ...
  return (
    <div className="w-100 d-flex flex-column align-items-center gap-4 p-4">
      ...
      <table className="table table-bordered">
        <tbody>
          <tr>
            ...
            <td className="py-2 px-4">
              <span>{total_answers}</span>
            </td>
          </tr>
          <tr>
            <td className="py-2 px-4">
              <span className="fw-semibold">Right Answers: </span>
            </td>
            <td className="py-2 px-4">
              <span>{true_answers}</span>
            </td>
          </tr>
          <tr>
            <td className="py-2 px-4">
              <span className="fw-semibold">Wrong Answers: </span>
            </td>
            <td className="py-2 px-4">
              <span>{false_answers}</span>
            </td>
          </tr>
          <tr>
            <td className="py-2 px-4">
      ...
      <button
        className="btn btn-outline-primary px-4 py-2 fs-5 mt-3"
...
export const Level2 = ({
  ...
  useEffect(() => {
    const allAnswered =
      !result.includes(undefined) && result?.length == questions?.length;
    ...
    <>
      <div className="w-100 d-flex flex-column align-items-center gap-4 p-4 mt-4">
        ...
        <div className="align-self-start w-100 d-flex flex-column gap-4">
          <div className="ps-2">
            <MuiMarkdown>{quiz?.description}</MuiMarkdown>
          </div>
          <table className="table table-bordered">
            <tbody>
              <tr>
              ...
              <tr>
                ...
                <td className="py-2 px-4">
                  <span>{section?.title || "Course"}</span>
                </td>
              </tr>
              <tr>
                <td className="py-2 px-4">
                ...
                <td className="py-2 px-4">
                  <span>{questions?.length || "No Questions"}</span>
                </td>
              </tr>
              <tr>
                <td className="py-2 px-4">
                  <span className="fw-semibold">Passing Marks: </span>
                </td>
                <td className="py-2 px-4">
                  <span>{`70%`}</span>
                </td>
              ...
              <tr>
                <td className="py-2 px-4">
                  <span className="fw-semibold">Instructor Name: </span>
                </td>
                <td className="py-2 px-4">
                  <span>
...
export const SingleLecture = ({ lecture, isLast, index }: any) => {
  return (
    <div className="col-12 col-md-4 mb-4">
      <div className="shadow-md border border-secondary rounded-4 border-1 shadow-sm border-opacity-10 ps-4 d-flex flex-column align-items-start gap-2 h-100">
        <div className="d-flex align-items-center gap-3 flex-shrink-0 my-3">
          <div className="calendar-card">
            <div className="calendar-header">
              {new Date(lecture?.class_date).toLocaleString("en-US", {
                month: "long",
              })}
            </div>
            <div className="calendar-pin"></div>
            <div className="calendar-date">
              {new Date(lecture?.class_date)
                .getDate()
                .toString()
                .padStart(2, "0")}
            </div>
          ...
          <div className="d-flex flex-column gap-2">
            <p className="mb-0 fw-semibold">{lecture?.title}</p>
            <div className="d-flex flex-row align-items-center gap-2 text-secondary">
              <span>🕒</span>
              <p className="mb-0">{moment(lecture?.class_time, 'HH:mm').format('hh:mm A')}</p>
              <p className="mb-0">{moment(lecture?.class_date).format('D MMM YYYY')}</p>
            </div>
            {lecture?.duration && (
...
const Lecture = ({ section_id, singleCourse, onLectureCount }: any) => {
  ...
  const getLectures = async () => {
    ...
    try {
      const resp = await axios.get(
        `${baseUrl}/live-classes/by-section/${section_id}`,
        authorizationObj
      );
      if (resp?.data?.data?.length) {
        setLectures(resp.data.data);
        if (onLectureCount) onLectureCount(resp.data.data.length);
      } else {
        setLectures([]);
        if (onLectureCount) onLectureCount(0);
      }
    } catch (error) {
      setLectures([]);
      if (onLectureCount) onLectureCount(0);
      ...
      <div className="row g-4">
        {timelineItems.map((item, index) =>
          item.type === "lecture" ? (
            ...
            <div
              key={`quiz-${item.data.quiz_id}`}
              className="col-12"
            >
              <div className="timeline-item ps-4 mt-3 shadow-sm rounded-4">
                <div className="d-flex flex-column flex-md-row align-items-start align-items-md-center gap-3">
                  <div className="calendar-card flex-shrink-0">
                    <div className="calendar-header">
                      March
                    </div>
                    <div className="calendar-pin"></div>
                  ...
                  <div
                    className="timeline-content bg-white rounded-3 p-3 w-100"
                    style={{
                  ...
                  >
                    <div className="d-flex flex-column gap-2">
                      <div className="d-flex flex-column flex-sm-row align-items-start align-items-sm-center gap-2">
                        <p className="mb-0 fw-semibold heading-style">
                          {item.data.title || "Quiz"}
                        </p>
                        <div className="ms-sm-3 d-flex align-items-center gap-2 text-muted small">
                          <span>📝</span>
                          <span>Quiz Assessment</span>
                        </div>
                      </div>
...
const AccordionComponent = ({ course }: any) => {
  ...
  // Track lecture counts for each section
  const [sectionLectureCounts, setSectionLectureCounts] = useState<{ [key: string]: number }>({});
  ...
  // Helper: get total classes for a section
  const getSectionClassCount = (section: any) => {
    return Array.isArray(section?.lectures) ? section.lectures.length : 0;
  };

  const handleLectureCount = (sectionId: string, count: number) => {
    setSectionLectureCounts((prev) => ({ ...prev, [sectionId]: count }));
  };
    ...
    <div className="container-fluid py-4">
      <div className="accordion" id="courseAccordion">
        {course_sections?.map((section, i) => (
          <div
            className="accordion-item mb-4 position-relative border-top-0 border-bottom-0 accordion-border border-right-0"
            key={i}
          >
              ...
              >
                <div className="d-flex flex-column w-100">
                  ...
                  <div className="d-flex flex-row ">
                    {/* <div className=" bg-light text-dark p-3 rounded border">
                      16 may 2025
                    </div> */}
                    ...
                    <div className="d-flex flex-column align-items-start ms-4">
                      ...
                      <div className="d-flex align-items-center gap-2 text-secondary">
                        <span>{sectionLectureCounts[section?.section_id] || 0} Classes</span>
                        <span>🎓</span>
                        <span>Unit {i + 1}</span>
                        {/* <span>🕒</span>
              ...
              <div className="accordion-body ms-4">
                <div className="mt-3">
                  <div className="row">
                    ...
                    <div className="col-12 col-md-12">
                      <h6 className="my-3 fw-semibold">Content</h6>
                      <Lecture
                        section_id={section?.section_id}
                        singleCourse={course}
                        onLectureCount={(count: number) => handleLectureCount(section?.section_id, count)}
                      />
                    </div>
                  </div>
