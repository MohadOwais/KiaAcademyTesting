...
const CertificatesTab = ({ userId }: { userId: string }) => {
  ...
  useEffect(() => {
    ...
    axios
      .get(
        `${baseUrl}/certificates/issuedto/${userId}`,
        authorizationObj
      )
      .then((resp) => {
  ...
  if (loading) return <div className="text-center py-5"><span className="spinner-border" /> Loading certificates...</div>;
  if (error) return <div className="alert alert-danger">{error}</div>;
  if (selectedCert) {
    return <CertificateViewer cert={selectedCert} onClose={() => setSelectedCert(null)} />;
  }
  if (!certificates.length) {
    return <div className="text-center text-secondary py-5">No certificates found.</div>;
  }
  return (
    <div className="row g-3">
      {certificates.map(cert => (
        <div className="col-12 col-md-6 col-lg-4" key={cert.certificate_id}>
          <CertificateCard
            cert={{
              ...cert,
              ...
              imageUrl: cert.course_thumbnail,
              description: `Issued by ${cert.issuer_name}${cert.instructor_name ? `, Instructor: ${cert.instructor_name}` : ''}`,
            }}
